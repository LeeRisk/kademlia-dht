...
class LookupActor(kBucketActor: ActorRef, reqSender: ActorRef)(implicit val config: KadConfig) extends FSM[State, Data] {
  import context._
  import config._
  ...
  when(QueryNode)(transform {
    case Event(KClosestReply(sender, nodes), data @ QueryNodeData(id, query, seen, count, currRound, _)) =>
      val updateNodeStatus = query + (sender -> query.get(sender).get.copy(queried = true)) // guaranteed to be in the queried map
  ...
  private def sendRequestInQuerying(queryData: QueryNodeData) { // send requests to nodes in querying that have not been queried yet
    import queryData._
    querying.foreach { case (_, NodeQuery(ref, queried, _)) => (reqSender ? NodeRequest(ref, KClosestRequest(config.id, req.id, kBucketSize))).mapTo[KClosest] pipeTo self }
  }
}
...
object LookupActor {
  ...
  private[kademlia] case class Lookup(id: Id, sender: ActorRef)
  ...
  case class QueryNodeData(req: Lookup, querying: Map[Id, NodeQuery] = Map(), seen: SortedMap[Id, NodeQuery], responseCount: Int = 0, currRound: Int = 1, lastRound: Boolean = false) extends Data
